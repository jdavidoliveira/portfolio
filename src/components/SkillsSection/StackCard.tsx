...
export default function StackCard({ name, photoUrl, children }: StackCardProps) {
    return (
        <div className="flex flex-col items-center justify-between w-16 h-18 sm:w-20 sm:h-24 hover:scale-105 transition duration-250 group">
            <div className="w-full flex items-center justify-center">
                <Image width={500} height={500} src={photoUrl} alt="Icone da stack" className="sm:w-5/6 w-4/6 aspect-square object-contain" />
            </div>
            <span className="w-full text-center text-white text-sm sm:text-base font-sans font-medium">{name}</span>
            {children && <HoverCard />}
        </div>
    )
